...
{{ /if }}
        ...
        <li class="commented">{{ include file="_tpl/relative_date.tpl" date=$created }} {{ #newCommentOn# }} <a href="{{ $gimme->community_feed->comment->article->url }}#comments">{{ $gimme->community_feed->comment->article->title }}</a></li>
        {{ /if }}
